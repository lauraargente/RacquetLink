...
function compareDates(doc1, doc2) {
  let date1 = new Date(doc1.clubRegisterDate);
  let date2 = new Date(doc2.clubRegisterDate);
...
function checkFilters(coach, filterCriteria) {
  ...
  if (
    ...
    if (
      !filterCriteria.dataSports.some((sport) =>
        coach.clubSports.includes(sport)
      )
    ) {
      return false;
    }
  ...
  if (
    filterCriteria.dataState.length > 0 &&
    filterCriteria.dataState[0] !== "all"
  ) {
  ...
  if (
    filterCriteria.dataConsulting.length > 0 &&
    filterCriteria.dataConsulting[0] !== "all"
  ) {
